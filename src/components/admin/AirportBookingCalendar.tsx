...
import { 
  ...
  DollarSign, ArrowRight, Star, HelpCircle, ClipboardList, Plane
} from 'lucide-react';
import { useApp } from '../../AppContext';
import { Booking } from '../../types';

export default function AirportBookingCalendar() {
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(() => {
    const stored = localStorage.getItem('smartjourney_selected_date_airport');
    if (stored) {
  ...
  // Filter bookings of type 'airport'
  ...
  // Helper: Get bookings scheduled on a specific date
  const getBookingsForDate = (dateStr: string) => {
    return airportBookings.filter(b => b.details?.date === dateStr);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          <h3 className="text-base font-black uppercase tracking-wider font-mono text-amber-500 flex items-center gap-2">
            ...
            AIRPORT TRANSFER BOOKING CALENDAR
          </h3>
          <p className="text-xs text-neutral-400">
            Kalender operasional penjemputan &amp; pengantaran bandara. Pantau slot ketersediaan per hari dan kelola penjemputan tamu secara real-time.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono font-bold bg-amber-500/10 border border-amber-500/30 text-amber-400 px-3 py-1 rounded-full uppercase">
            Limit Harian: {serviceLimits?.airport ?? 5} Slots
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-4 bg-neutral-900 border border-neutral-800 rounded-2xl p-6 h-full flex flex-col">
          {/* Calendar Header Controls */}
          <div className="flex justify-between items-center border-b border-neutral-800 pb-4">
            <div className="flex items-center gap-2.5">
              <CalendarIcon className="h-5 w-5 text-amber-500" />
              <h2 className="text-sm font-black uppercase tracking-wide font-mono text-neutral-100">
            ...
            <div className="flex items-center gap-1.5">
              <button 
                ...
                className="px-3 py-1.5 hover:bg-neutral-850 text-[10px] font-mono font-black border border-neutral-800 text-amber-500 hover:text-amber-400 rounded-lg transition-all cursor-pointer uppercase"
              >
                Hari Ini
              </button>
              <button 
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem('smartjourney_selected_date_airport', cellDate.toISOString());
            ...
            {selectedDateBookings.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="space-y-2 bg-neutral-900/50 p-3.5 rounded-xl border border-neutral-850/60 text-xs">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <span className="text-[9px] text-neutral-500 font-bold block uppercase tracking-wider font-mono">Titik Jemput:</span>
                          <span className="text-neutral-200 font-bold leading-tight">{details.pickupLocation || 'Bandara'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <span className="text-[9px] text-neutral-500 font-bold block uppercase tracking-wider font-mono">Tujuan Antar:</span>
                          <span className="text-neutral-200 font-bold leading-tight">{details.destination || details.cityAddress}</span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-12 px-4 border-2 border-dashed border-neutral-800 rounded-2xl bg-neutral-950/20">
                ...
                <p className="text-[11px] text-neutral-500 mt-1 max-w-xs leading-relaxed">
                  Belum ada penjemputan bandara yang dijadwalkan untuk tanggal {selectedDate.getDate()} {monthNames[selectedDate.getMonth()]} {selectedDate.getFullYear()}.
                </p>
              </div>
            )}
          </div>
