...
export default function FloatingWhatsApp() {
  ...
  return (
    <div className="fixed bottom-3 right-3 sm:bottom-6 sm:right-6 z-50 flex flex-col gap-2.5 items-end">
      {/* Scroll to Top Button (directly above the WhatsApp button) */}
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          aria-label="Kembali ke atas"
          className="bg-white hover:bg-neutral-100 text-neutral-800 border border-neutral-200/85 p-2.5 sm:p-3 rounded-full shadow-lg hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center relative group cursor-pointer"
        >
      ...
      <a
        ...
        className="bg-[#25D366] hover:bg-[#20ba5a] p-3 sm:p-4 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center relative group"
      >
        {/* Actual official-looking WhatsApp Icon */}
        <svg
          viewBox="0 0 448 512"
          className="h-5 w-5 sm:h-6 sm:w-6 text-white fill-current"
